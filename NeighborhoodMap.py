...
win = g.GraphWin('Map', 1200, 720) # give title and dimensions
#figure out widths
x0 = win.getWidth()/2
y0 = win.getHeight()/2
#how many chunks?
...
xmap = (1-flatscale)*np.random.rand(4*bx,4*by)
for i in range(4*bx):
    for j in range(4*by):
...
for i in range(-2*bx,2*bx):
    for j in range(-2*by,2*by):
        ...
        if(i%2==0 and ymap[2*(i//2),j]<py): #removed vertical roads
        ...
        elif(j%2==0 and xmap[i,2*(j//2)]<px): #removed horizontal roads
            #draw housesY
